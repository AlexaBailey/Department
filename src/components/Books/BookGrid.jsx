...
const BooksGrid = () => {
  ...
  useEffect(() => {
    if (data?.books?.length) {
      setBooks((prevBooks) => {
        if (direction === "down") {
          return [...prevBooks.slice(5), ...data.books];
        } else {
          return [...data.books, ...prevBooks.slice(0, 5)];
        }
      });
    }
    setIsFetching(false);
  ...
  const scrollHandler = () => {
    ...
    if (scrollHeight - scrollTop - clientHeight < 50) {
      setDirection("down");
      setIsFetching(true);
      setCurrentStart((prevStart) => prevStart + 5);
    }

    if (scrollTop < 50 && currentStart > 0) {
      setDirection("up");
      setIsFetching(true);
      setCurrentStart((prevStart) => (prevStart > 0 ? prevStart - 5 : 0));
    }
  };
